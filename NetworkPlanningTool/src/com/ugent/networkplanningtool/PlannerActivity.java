 ...
 */
public class PlannerActivity extends Activity implements Observer, OnTouchListener {

    private static final String TAG = PlannerActivity.class.getName();

    private static PlannerActivity mContext;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        floorPlanModel = FloorPlanModel.PLANNING_INSTANCE;

        drawingView = (DrawingView) findViewById(R.id.drawingView);
        drawingModel = new DrawingModel(drawingView.getWidth(), drawingView.getHeight(),floorPlanModel);
    ...
    public void onMainFlipClick(View v) {
        if (v.getId() == R.id.resultsButton) {
            if (floorPlanModel.getDeusResult() == null) {
                Toast.makeText(PlannerActivity.this, "No result to display.", Toast.LENGTH_SHORT).show();
                return;
            } else {
    ...
    public void onResultsFlipClick(View v) {
        ...
        if (!canMeasure && v.equals(findViewById(R.id.measureButton))) {
            Toast.makeText(PlannerActivity.this, "Can only perform measurements for predicted coverage", Toast.LENGTH_LONG).show();
            ...
            public void onFileSelected(Dialog source, File file) {
                ...
                taskManager.executeTask(new LoadFloorPlanTask(), file, "Loading" + file.getName() + " ...", new OnAsyncTaskCompleteListener<FloorPlan>() {
                    ...
                    public void onTaskCompleteSuccess(FloorPlan result) {
                        ...
                        Toast.makeText(PlannerActivity.this, "Floorplan loaded", Toast.LENGTH_LONG).show();
                    }

                    @Override
                    public void onTaskFailed(Exception cause) {
                        Log.e(TAG, cause.getMessage(), cause);
                        Toast.makeText(PlannerActivity.this, cause.getMessage(), Toast.LENGTH_LONG).show();
                    }
                }, false);
            }
        });
    ...
    public void saveTofile(SaveXMLParams params) {
        taskManager.executeTask(new SaveXMLTask(), params, "saving...", new OnAsyncTaskCompleteListener<File>() {
            ...
            public void onTaskCompleteSuccess(File result) {
                Toast.makeText(PlannerActivity.this, "Saved successful to " + result.getName(), Toast.LENGTH_LONG).show();
            }
     ...
     * @return the instance
     */
    public static PlannerActivity getInstance() {
        return mContext;
    }
    ...
    public void handleImportImage(View v) {
        ...
        dialog.addListener(new OnFileSelectedListener() {
            ...
            public void onFileSelected(Dialog source, File file) {
                ...
                final ImportImage iiDialog = new ImportImage(PlannerActivity.this);
    ...
    public void handleLinkingConfig(final View view) {
        ...
        taskManager.executeTask(new WifiDetectTask(this), wifi, "Detecting wifi signals...", new OnAsyncTaskCompleteListener<List<RealAccessPoint>>() {
            ...
            public void onTaskCompleteSuccess(List<RealAccessPoint> result) {
                final Dialog d = new Dialog(PlannerActivity.this);
                d.setContentView(new ApLinkingView(PlannerActivity.this, result));
                displayNewDialog(d);
            }
            ...
            public void onFileSelected(Dialog source, File file) {
                ...
                taskManager.executeTask(new LoadMeasurementsTask(), file, "Loading" + file.getName() + " ...", new OnAsyncTaskCompleteListener<List<ApMeasurement>>() {
                    ...
                    public void onTaskCompleteSuccess(List<ApMeasurement> result) {
                        ...
                        Toast.makeText(PlannerActivity.this, "measurements loaded", Toast.LENGTH_LONG).show();
                    }
    ...
    public void oneApplyMeasurementsClick(final View v) {
        ...
        if (apMeasurements.size() > 0) {
            ...
            Toast.makeText(PlannerActivity.this, "Shift of " + shift + " applied. Results can be checked at the powerRX results.", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(PlannerActivity.this, "There are no measurements", Toast.LENGTH_SHORT).show();
        }
...
//        saveTofile(saveXMLParams);
    ...
    public void handleSaveRawData(View view) {
        FileChooserDialog dialog = new FileChooserDialog(PlannerActivity.this);
        dialog.addListener(new OnFileSelectedListener() {
            @Override
            public void onFileSelected(Dialog source, File folder, String name) {
                String extension = "";
        ...
        switch (exportType) {
            case NORMALIZED_PLAN:
                if (dr.getNormalizedPlan() != null) {
                    ...
                    Toast.makeText(PlannerActivity.this, "No normalized plan available.", Toast.LENGTH_SHORT).show();
                }
                break;
            case OPTIMIZED_PLAN:
                if (dr.getOptimizedPlan() != null) {
                    ...
                    Toast.makeText(PlannerActivity.this, "No optimized plan available.", Toast.LENGTH_SHORT).show();
                }
                break;
            case COVERAGE_DATA:
                if (dr.getCsv() != null) {
                    ...
                    Toast.makeText(PlannerActivity.this, "No measure results available.", Toast.LENGTH_SHORT).show();
                }
                break;
            case EXPOSURE_INFO:
                if (dr.getInfoAsString() != null) {
                    ...
                    Toast.makeText(PlannerActivity.this, "No info available.", Toast.LENGTH_SHORT).show();
                }
                break;
            case BENCHMARK:
                if (dr.getBenchmarks() != null) {
                    ...
                    Toast.makeText(PlannerActivity.this, "No benchmarks available.", Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
