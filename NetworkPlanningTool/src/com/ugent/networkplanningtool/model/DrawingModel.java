...
public class DrawingModel extends Observable {
    ...
    public static enum PlaceResult {
        SUCCESS(null),
        CONNECTION_POINT_NOT_ADJACENT_TO_WALL(PlannerActivity.getInstance().getResources().getString(R.string.connectionPointNotAdjacentToWall)),
        NOTHING_TO_PLACE(PlannerActivity.getInstance().getResources().getString(R.string.nothingToPlace));
     ...
     */
    public DrawingModel(int viewWidth, int viewHeight, FloorPlanModel model) {
        this.model = model;
        offsetX = 0;
        offsetY = 0;
            ...
            if (touchFloorPlanObject instanceof Wall) {
                ...
                if (touchFloorPlanObject.isComplete()) {
                    if (touchFloorPlanObject instanceof ConnectionPoint) {
                        Couple<Double, Wall> closestWallCouple = model.getClosestWallToPoint(touchFloorPlanObject.getPoint1(), true);
                        if (closestWallCouple != null) {
                            double dist = closestWallCouple.getA();
                            if (dist <= INTERVAL / 4 && dist != 0) {
                                model.addFloorPlanObject(touchFloorPlanObject);
                                touchFloorPlanObject = touchFloorPlanObject.getPartialDeepCopy();
                                setChanged();
                                notifyObservers();
    ...
    public void setTouchLocation(float x, float y) {
        if (touchFloorPlanObject != null) {
            ...
            if (touchFloorPlanObject instanceof Wall) {
                Couple<Double, Point> closestCouple = model.getClosestCornerToPoint(wallPoint);
                if (closestCouple.getB() != null && closestCouple.getA() <= INTERVAL / 2) {
                    ...
                    switch (snapTo) {
                        case GRID:
                            ...
                            break;
                        case WALLS:
                            Couple<Double, Wall> closestWallCouple = model.getClosestWallToPoint(wallPoint, false);
    ...
    public void select(float x, float y, boolean select) {
        ...
        // get closest
        Couple<Double, FloorPlanObject> closestCouple = model.getClosestFloorPlanObjectToPoint(touchLocation, select);
