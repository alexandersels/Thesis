...
public class DrawingView extends View implements Observer {
    ...
    public DrawingView(Context context, AttributeSet attrs) {
        super(context, attrs);
        FloorPlanModel.PLANNING_INSTANCE.addObserver(this);
    ...
    protected void onDraw(Canvas canvas) {
        if (drawingModel != null) {
            ...
            if(drawingModel.isDrawResult()){
                ...
                DeusResult dr = FloorPlanModel.PLANNING_INSTANCE.getDeusResult();
                ...
                for (ApMeasurement apMeasurement : FloorPlanModel.PLANNING_INSTANCE.getApMeasurements()) {
    ...
    private void drawAccessPoints(Canvas canvas) {
        List<AccessPoint> accessPointList = FloorPlanModel.PLANNING_INSTANCE.getAccessPointList();
    ...
    private void drawConnectionPoints(Canvas canvas) {
        List<ConnectionPoint> connectionPointList = FloorPlanModel.PLANNING_INSTANCE.getConnectionPointList();
    ...
    private void drawActivities(Canvas canvas) {
        List<DataActivity> activityList = FloorPlanModel.PLANNING_INSTANCE.getDataActivityList();
    ...
    private void drawWalls(Canvas canvas) {
        List<Wall> wallList = FloorPlanModel.PLANNING_INSTANCE.getWallList();
        for (Wall w : wallList) {
            ...
            case MotionEvent.ACTION_UP:
                switch (drawingModel.getState()) {
                        ...
                        if (drawingModel.getTouchFloorPlanObject() instanceof ApMeasurement) {
                            ...
                            adb.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    MeasureParams mp = new MeasureParams(apm.getSamplePoolSize(), FloorPlanModel.PLANNING_INSTANCE.getAccessPointList());
                                    PlannerActivity.getInstance().getTaskManager().executeTask(new MeasureSignalStrengthTask(getContext()), mp, "Sampling...", new OnAsyncTaskCompleteListener<Integer>() {
                                        @Override
                                        public void onTaskCompleteSuccess(Integer result) {
                                            apm.setSignalStrength(result);
                                            PlaceResult pr = drawingModel.place();
                                            CSVResult closestResult = Utils.getResultAt(apm.getPoint1(), FloorPlanModel.PLANNING_INSTANCE.getDeusResult().getCsv());
                    ...
                    case MEASURE_REMOVE:
                        dObj = drawingModel.getSelected();
                        if (dObj != null) {
                            FloorPlanModel.PLANNING_INSTANCE.removeFloorPlanObject(dObj);
                        }
                        drawingModel.deselect();
                        break;
                    default:
